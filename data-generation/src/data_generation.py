...
for i in range(3):
    ...
    data_OPE_CONDUCTORES.append((rn_ope_conduc, fake.first_name(), fake.last_name(), random.choice(tip_doc), doc_hash, 
                 fake.date_between(start_date='-10y', end_date="now"), 
                 random.choice(list(ciudad_base)), random.choice(tip_vehiculo), 
                 random.choice(id_zona), random.choice(activo), cal_promedio_acumulado)) 


print(data_OPE_CONDUCTORES)
print("\n")
...
def nombre_zona(id_ciudad_aleatorio):
    if id_ciudad_aleatorio == "BOG" or id_ciudad == "CAL" or id_ciudad == "MDE" or id_ciudad == "BGA" or id_ciudad == "PER" or id_ciudad == "MZL" or id_ciudad == "CUC":
...
for i in range(10):
    ...
    data_geo_zona.append((id_zona[i], nombre_zona(id_ciudad_aleatorio), id_ciudad_aleatorio,barrio_referencia(id_ciudad_aleatorio), 
...
for i in range(3):
    ...
    fec_recepcion = fake.date_between(start_date='-10y', end_date='now')
    ...
    data_TMS_envios.append((rn_tms_envios, random.choice(ids_r), random.choice(ids_c), random.choice(id_zona),
                            random_tip_paquete, peso(random_tip_paquete), *resultado_fechas))

print(data_TMS_envios, "\n")
...
for i in range(3):
    ...
    desviacion_ruta_km = round(random.uniform(0,100000), 2)
    consumo_combustible = ["Alto", "Bajo", "Medio"]

    data_GPS_rutas.append((rn_gps_rutas, random.choice(ids_c),resultado_fechas[2],hora_inicio, hora_fin, km_recorridos, num_paradas_plan, num_paradas_real, desviacion_ruta_km,random.choice(consumo_combustible)))

print(data_GPS_rutas, "\n")
...
for i in range(3):
    ...
    comentario = random.choice([fake.text(max_nb_chars=50), None])
